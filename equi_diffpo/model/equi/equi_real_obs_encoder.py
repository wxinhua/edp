...
class EquivariantMultiObsEnc(ModuleAttrMixin):
    def __init__(
        self,
        obs_shape=(3, 480, 640),
        crop_shape=(240, 320),
        ...
        self.enc_out = nn.Linear(
            nn.FieldType(
                self.group,
                3 * n_hidden * [self.group.regular_repr] # agentview
                + 8 * [self.group.trivial_repr], # 8d joint pos
            ),
            self.token_type,
        )
    ...
    def forward(self, nobs):
        # print(f"nobs['rgb'] len is :{len(nobs['rgb'])}, nobs['rgb'][0] len is:{len(nobs['rgb'][0])}")
        # print(f"nobs['rgb'][0][0] len is:{len(nobs['rgb'][0][0])}")
        # print(f"nobs['rgb'] size is :{nobs['rgb'].size()}")
        # Extract views while keeping batch and time dimensions
        obs_1 = nobs["rgb"][:, 0]  # First view, shape: [batch_size, time, channels, height, width]
        ...
        t = obs_1.shape[1]
        obs_1 = rearrange(obs_1, "b t c h w -> (b t) c h w")
        obs_2 = rearrange(obs_2, "b t c h w -> (b t) c h w")
        obs_3 = rearrange(obs_3, "b t c h w -> (b t) c h w")
        joint_pos = rearrange(joint_pos, "b t d -> (b t) d")
